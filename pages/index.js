...
export default function Home() {
  ...
  async function validateAdminPassword(pass) {
    ...
    try {
      const r = await fetch('/api/validate-admin', {
        method: 'POST',
        headers: {
      ...
      if (!r.ok) {
        localStorage.removeItem('dmi_admin_pass');
        setIsAdmin(false);
        setAdminChecked(true);
        setMsg(j.error || 'Not authorised');
        return false;
      }

      localStorage.setItem('dmi_admin_pass', pass);
      setAdmin(pass);
      setIsAdmin(true);
      setAdminChecked(true);
      return true;
  ...
  useEffect(() => {
    ...
    const saved =
      typeof window !== 'undefined'
        ? localStorage.getItem('dmi_admin_pass')
        : null;

    if (saved) {
      setAdmin(saved);
      validateAdminPassword(saved);
    } else {
      setAdminChecked(true);
    }
  }, []);
  ...
  const pot =
    entries.filter(e => e.paid).length * Number(settings.entry_fee || 0);
          ...
          <section className="card adminLock">
            ...
            {!adminChecked ? (
              ...
              <div className="adminLogin">
                ...
                <button
                  className="primary"
                  onClick={() => validateAdminPassword(admin)}
                >
                  Unlock Admin
                </button>
              </div>
...
function OldSchool({ week, fixtures, settings, maxPts }) {
  return (
    <section className="paper">
      ...
      <p>{settings.rules}</p>
      <p>
        <b>1 point</b> correct result | <b>3 points</b> correct score | Max{' '}
        {maxPts}
      </p>
          ...
          <section className="card">
            ...
            {!isAdmin ? (
              <>
                ...
                <button
                  onClick={() => {
                    localStorage.setItem('dmi_admin_pass', admin);
                    setIsAdmin(true);
                  }}
                >
                  Log in
